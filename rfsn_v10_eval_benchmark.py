from __future__ import annotations

"""
RFSN v10.2 - Dense vs tiered-cache evaluation benchmark.

This script compares exact dense attention against the tiered cache path on
synthetic long sequences. It evaluates both PQ-only and PQ+RVQ modes across a
context-window sweep and across storage profiles that include hot+warm-only and
hot+warm+cold scenarios. Each row includes cache access instrumentation so the
measured latency can be tied back to actual reconstructed tokens and decoded
warm/cold chunks.
"""

import argparse
import csv
import logging
import shutil
import time
from dataclasses import replace
from pathlib import Path
from typing import Dict, List, Optional, Sequence, Tuple

import mlx.core as mx
import numpy as np

from attention import _blockwise_exact_attention
from cache import RFSNv10KVCacheMLX
from quantization import HybridQuantizerMLX
from storage import AsyncHierarchicalRouterMLX, RFSNConfig


logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
)
logger = logging.getLogger("RFSN_EVAL")


def _dense_bytes(config: RFSNConfig, seq_len: int) -> int:
    return seq_len * config.num_heads * config.head_dim * 2 * 2


def _cache_bytes(cache: RFSNv10KVCacheMLX) -> int:
    mem = cache.memory_usage_bytes()
    return mem["hot_bytes"] + mem["warm_pq_bytes"] + mem["warm_rvq_bytes"]


def _cold_disk_bytes(cache: RFSNv10KVCacheMLX) -> int:
    return sum(path.stat().st_size for path in cache.cold_chunk_paths if path.exists())


def _window_bounds(query_positions: mx.array, context_window: Optional[int]) -> Tuple[int, int, mx.array]:
    query_np = np.asarray(query_positions, dtype=np.int64)
    max_query = int(query_np.max())
    window_end = max_query + 1
    if context_window is None:
        window_start = 0
    else:
        window_start = max(0, window_end - context_window)
    adjusted = query_positions.astype(mx.int32) - window_start
    return window_start, window_end, adjusted.astype(mx.int32)


def _materialize_dense_window(
    keys: mx.array,
    values: mx.array,
    query_positions: mx.array,
    context_window: Optional[int],
) -> Tuple[mx.array, mx.array, mx.array]:
    window_start, window_end, adjusted = _window_bounds(query_positions, context_window)
    return keys[window_start:window_end], values[window_start:window_end], adjusted


def _score_table(q: mx.array, keys: mx.array) -> mx.array:
    return mx.sum(
        q.astype(mx.float32)[:, :, None, :] * mx.swapaxes(keys.astype(mx.float32), 0, 1)[None, :, :, :],
        axis=-1,
    )


def _masked_score_metrics(
    dense_scores: mx.array,
    reconstructed_scores: mx.array,
    query_positions: mx.array,
) -> Dict[str, float]:
    dense_np = np.asarray(dense_scores, dtype=np.float32)
    recon_np = np.asarray(reconstructed_scores, dtype=np.float32)
    query_np = np.asarray(query_positions, dtype=np.int64)
    seq_len = dense_np.shape[-1]
    positions = np.arange(seq_len, dtype=np.int64)[None, None, :]
    valid = positions <= query_np[:, None, None]
    delta = dense_np - recon_np
    valid = np.broadcast_to(valid, delta.shape)
    valid_delta = delta[valid]
    return {
        "score_mae": float(np.mean(np.abs(valid_delta))),
        "score_rmse": float(np.sqrt(np.mean(valid_delta ** 2))),
    }


def _time_call(fn, repeats: int) -> Tuple[float, mx.array]:
    output = fn()
    mx.eval(output)
    start = time.perf_counter()
    for _ in range(repeats):
        output = fn()
        mx.eval(output)
    elapsed_ms = ((time.perf_counter() - start) * 1000.0) / repeats
    return elapsed_ms, output


def _copy_codebooks(source: HybridQuantizerMLX, target: HybridQuantizerMLX) -> None:
    target.pq.codebooks = mx.array(np.asarray(source.pq.codebooks), dtype=mx.float16)
    target.rvq.codebooks = mx.array(np.asarray(source.rvq.codebooks), dtype=mx.float16)


def _build_mode_quantizers(config: RFSNConfig) -> List[Tuple[str, HybridQuantizerMLX]]:
    pq_rvq = HybridQuantizerMLX(config)
    pq_only = HybridQuantizerMLX(replace(config, rvq_sparsity_threshold=float("inf")))
    _copy_codebooks(pq_rvq, pq_only)
    return [("pq_only", pq_only), ("pq_rvq", pq_rvq)]


def _make_cache_dir(
    eval_disk_root: Path,
    storage_profile: str,
    seq_len: int,
    requested_context_window: int,
    mode_name: str,
) -> Path:
    return eval_disk_root / f"{storage_profile}_len{seq_len}_window{requested_context_window}_{mode_name}"


def _evaluate_mode(
    mode_name: str,
    quantizer: HybridQuantizerMLX,
    config: RFSNConfig,
    keys: mx.array,
    values: mx.array,
    q: mx.array,
    query_positions: mx.array,
    repeats: int,
    context_window: Optional[int],
    storage_profile: str,
    use_router: bool,
    cache_dir: Path,
) -> Dict[str, float]:
    cache = RFSNv10KVCacheMLX(config, layer_idx=0)
    if cache_dir.exists():
        shutil.rmtree(cache_dir)
    cache.update(keys, values, quantizer, disk_dir=cache_dir)

    current_position = int(np.max(np.asarray(query_positions, dtype=np.int64)))
    router_enabled = use_router and context_window is not None and cache.num_cold > 0
    router = AsyncHierarchicalRouterMLX(config, layer_idx=0) if router_enabled else None

    dense_keys, dense_values, adjusted_positions = _materialize_dense_window(
        keys,
        values,
        query_positions=query_positions,
        context_window=context_window,
    )
    reconstructed_keys, _, reconstructed_positions = cache.materialize_window(
        query_positions=query_positions,
        context_window=context_window,
        router=None,
        current_position=current_position,
    )

    dense_fn = lambda: _blockwise_exact_attention(
        q=q,
        keys=dense_keys,
        values=dense_values,
        scale=config.head_dim ** -0.5,
        block_size=config.block_size_seq,
        causal=True,
        query_positions=adjusted_positions,
    )
    cache_fn = lambda: cache.attention_forward(
        q,
        causal=True,
        query_positions=query_positions,
        router=router,
        current_position=current_position,
        context_window=context_window,
    )

    dense_latency_ms, dense_out = _time_call(dense_fn, repeats)
    cache_latency_ms, cache_out = _time_call(cache_fn, repeats)
    access_stats = cache.get_last_access_stats()

    output_delta = np.asarray(dense_out.astype(mx.float32) - cache_out.astype(mx.float32), dtype=np.float32)
    dense_scores = _score_table(q, dense_keys)
    reconstructed_scores = _score_table(q, reconstructed_keys)
    score_metrics = _masked_score_metrics(dense_scores, reconstructed_scores, reconstructed_positions)

    dense_total_bytes = _dense_bytes(config, int(keys.shape[0]))
    cache_total_bytes = _cache_bytes(cache)
    cold_disk_bytes = _cold_disk_bytes(cache)
    mem = cache.memory_usage_bytes()

    warm_vectors = max(cache.num_warm * config.num_heads, 1)
    key_active = int(cache.warm_keys.rvq_codes.shape[0])
    value_active = int(cache.warm_values.rvq_codes.shape[0])
    requested_context_window = 0 if context_window is None else context_window

    return {
        "sequence_length": int(keys.shape[0]),
        "storage_profile": storage_profile,
        "requested_context_window": requested_context_window,
        "window_tokens": access_stats["window_tokens"],
        "query_count": int(q.shape[0]),
        "quantizer_mode": mode_name,
        "rvq_enabled": 0 if mode_name == "pq_only" else 1,
        "uses_router": int(router_enabled),
        "hot_capacity": config.hot_capacity,
        "warm_capacity": config.warm_capacity,
        "cold_capacity": config.cold_capacity,
        "hot_tokens": cache.num_hot,
        "warm_tokens": cache.num_warm,
        "cold_tokens": cache.num_cold,
        "dense_latency_ms": dense_latency_ms,
        "cache_latency_ms": cache_latency_ms,
        "latency_ratio": cache_latency_ms / dense_latency_ms if dense_latency_ms else 0.0,
        "dense_bytes": dense_total_bytes,
        "cache_bytes": cache_total_bytes,
        "cold_disk_bytes": cold_disk_bytes,
        "total_footprint_bytes": cache_total_bytes + cold_disk_bytes,
        "memory_saved_bytes": dense_total_bytes - cache_total_bytes,
        "compression_ratio": dense_total_bytes / cache_total_bytes if cache_total_bytes else 0.0,
        "footprint_compression_ratio": dense_total_bytes / (cache_total_bytes + cold_disk_bytes) if (cache_total_bytes + cold_disk_bytes) else 0.0,
        "output_mae": float(np.mean(np.abs(output_delta))),
        "output_rmse": float(np.sqrt(np.mean(output_delta ** 2))),
        "output_max_abs": float(np.max(np.abs(output_delta))),
        "score_mae": score_metrics["score_mae"],
        "score_rmse": score_metrics["score_rmse"],
        "hot_bytes": mem["hot_bytes"],
        "warm_pq_bytes": mem["warm_pq_bytes"],
        "warm_rvq_bytes": mem["warm_rvq_bytes"],
        "rvq_key_active": key_active,
        "rvq_value_active": value_active,
        "rvq_key_active_fraction": key_active / warm_vectors,
        "rvq_value_active_fraction": value_active / warm_vectors,
        "window_start": access_stats["window_start"],
        "window_end": access_stats["window_end"],
        "hot_tokens_materialized": access_stats["hot_tokens_materialized"],
        "warm_tokens_materialized": access_stats["warm_tokens_materialized"],
        "cold_tokens_materialized": access_stats["cold_tokens_materialized"],
        "reconstructed_tokens": access_stats["reconstructed_tokens"],
        "reconstructed_token_fraction": access_stats["reconstructed_tokens"] / access_stats["window_tokens"] if access_stats["window_tokens"] else 0.0,
        "warm_chunk_decodes": access_stats["warm_chunk_decodes"],
        "cold_chunk_decodes": access_stats["cold_chunk_decodes"],
        "cold_chunks_touched": access_stats["cold_chunks_touched"],
        "cold_chunk_cache_hits": access_stats["cold_chunk_cache_hits"],
        "cold_chunk_cache_misses": access_stats["cold_chunk_cache_misses"],
    }


def _effective_context_windows(context_windows: Sequence[int]) -> List[Optional[int]]:
    normalized: List[Optional[int]] = []
    seen: set[int] = set()
    for value in context_windows:
        normalized_value = None if value <= 0 else value
        dedupe_key = 0 if normalized_value is None else normalized_value
        if dedupe_key in seen:
            continue
        seen.add(dedupe_key)
        normalized.append(normalized_value)
    return normalized


def _build_storage_profiles(
    max_len: int,
    hot_capacity: int,
    warm_capacity: Optional[int],
    cold_warm_capacity: Optional[int],
) -> List[Dict[str, int]]:
    profiles: List[Dict[str, int]] = []
    hot_warm_capacity = warm_capacity if warm_capacity is not None else max(1, max_len - hot_capacity)
    if max_len > hot_capacity + hot_warm_capacity:
        raise ValueError("All sequence lengths must fit into hot + warm capacity for the hot_warm profile")
    profiles.append(
        {
            "storage_profile": "hot_warm",
            "warm_capacity": hot_warm_capacity,
            "cold_capacity": 0,
            "uses_router": 0,
        }
    )

    if max_len > hot_capacity + 1:
        max_cold_warm = max_len - hot_capacity - 1
        default_cold_warm = min(64, max_cold_warm)
        effective_cold_warm = cold_warm_capacity if cold_warm_capacity is not None else default_cold_warm
        effective_cold_warm = max(1, min(max_cold_warm, effective_cold_warm))
        profiles.append(
            {
                "storage_profile": "cold_mixed",
                "warm_capacity": effective_cold_warm,
                "cold_capacity": max_len,
                "uses_router": 0,
            }
        )
    return profiles


def evaluate_sequence_length(
    seq_len: int,
    base_config: RFSNConfig,
    repeats: int,
    query_count: int,
    context_windows: Sequence[Optional[int]],
    storage_profiles: Sequence[Dict[str, int]],
    use_router_for_cold: bool,
    eval_disk_root: Path,
) -> List[Dict[str, float]]:
    eval_queries = min(query_count, seq_len)
    keys = mx.random.normal(shape=(seq_len, base_config.num_heads, base_config.head_dim), dtype=mx.float32).astype(mx.float16)
    values = mx.random.normal(shape=(seq_len, base_config.num_heads, base_config.head_dim), dtype=mx.float32).astype(mx.float16)
    q = mx.random.normal(shape=(eval_queries, base_config.num_heads, base_config.head_dim), dtype=mx.float32).astype(mx.float16)
    start_query = max(0, seq_len - eval_queries)
    query_positions = mx.array(np.arange(start_query, seq_len, dtype=np.int32), dtype=mx.int32)

    rows: List[Dict[str, float]] = []
    for profile in storage_profiles:
        profile_config = replace(
            base_config,
            warm_capacity=profile["warm_capacity"],
            cold_capacity=profile["cold_capacity"],
            disk_cache_dir=str(eval_disk_root / profile["storage_profile"]),
        )
        quantizers = _build_mode_quantizers(profile_config)
        for context_window in context_windows:
            if context_window is not None and context_window < eval_queries:
                raise ValueError("context_window must be at least as large as query_count")

            pair_rows: List[Dict[str, float]] = []
            for mode_name, quantizer in quantizers:
                cache_dir = _make_cache_dir(
                    eval_disk_root=eval_disk_root,
                    storage_profile=profile["storage_profile"],
                    seq_len=seq_len,
                    requested_context_window=0 if context_window is None else context_window,
                    mode_name=mode_name,
                )
                pair_rows.append(
                    _evaluate_mode(
                        mode_name=mode_name,
                        quantizer=quantizer,
                        config=profile_config,
                        keys=keys,
                        values=values,
                        q=q,
                        query_positions=query_positions,
                        repeats=repeats,
                        context_window=context_window,
                        storage_profile=profile["storage_profile"],
                        use_router=use_router_for_cold and profile["storage_profile"] == "cold_mixed",
                        cache_dir=cache_dir,
                    )
                )

            pq_only_row = next(row for row in pair_rows if row["quantizer_mode"] == "pq_only")
            for row in pair_rows:
                row["output_rmse_reduction_vs_pq_only"] = pq_only_row["output_rmse"] - row["output_rmse"]
                row["score_rmse_reduction_vs_pq_only"] = pq_only_row["score_rmse"] - row["score_rmse"]
                row["cache_latency_delta_ms_vs_pq_only"] = row["cache_latency_ms"] - pq_only_row["cache_latency_ms"]
            rows.extend(pair_rows)

    return rows


def run_evaluation(
    lengths: Sequence[int],
    repeats: int,
    query_count: int,
    hot_capacity: int,
    warm_capacity: Optional[int],
    cold_warm_capacity: Optional[int],
    output_path: Path,
    context_windows: Sequence[int],
    use_router_for_cold: bool,
) -> List[Dict[str, float]]:
    if hasattr(mx.random, "seed"):
        mx.random.seed(17)
    np.random.seed(17)

    max_len = max(lengths)
    normalized_context_windows = _effective_context_windows(context_windows)
    storage_profiles = _build_storage_profiles(
        max_len=max_len,
        hot_capacity=hot_capacity,
        warm_capacity=warm_capacity,
        cold_warm_capacity=cold_warm_capacity,
    )

    base_config = RFSNConfig(
        hidden_dim=512,
        num_heads=4,
        head_dim=128,
        num_layers=2,
        num_subspaces=8,
        subspace_dim=16,
        num_rvq_layers=3,
        rvq_codebook_size=256,
        rvq_sparsity_threshold=0.25,
        hot_capacity=hot_capacity,
        warm_capacity=max(1, max_len - hot_capacity),
        cold_capacity=max_len,
        block_size_seq=16,
        disk_cache_dir="./eval_disk_cache_unused",
        prefetch_throttle_s=0.0,
    )

    eval_disk_root = Path("./benchmark_rfsn_eval_disk_cache")
    if eval_disk_root.exists():
        shutil.rmtree(eval_disk_root)
    eval_disk_root.mkdir(parents=True, exist_ok=True)

    try:
        results: List[Dict[str, float]] = []
        for seq_len in lengths:
            results.extend(
                evaluate_sequence_length(
                    seq_len=seq_len,
                    base_config=base_config,
                    repeats=repeats,
                    query_count=query_count,
                    context_windows=normalized_context_windows,
                    storage_profiles=storage_profiles,
                    use_router_for_cold=use_router_for_cold,
                    eval_disk_root=eval_disk_root,
                )
            )
    finally:
        if eval_disk_root.exists():
            shutil.rmtree(eval_disk_root)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = [
        "sequence_length",
        "storage_profile",
        "requested_context_window",
        "window_tokens",
        "query_count",
        "quantizer_mode",
        "rvq_enabled",
        "uses_router",
        "hot_capacity",
        "warm_capacity",
        "cold_capacity",
        "hot_tokens",
        "warm_tokens",
        "cold_tokens",
        "dense_latency_ms",
        "cache_latency_ms",
        "latency_ratio",
        "cache_latency_delta_ms_vs_pq_only",
        "dense_bytes",
        "cache_bytes",
        "cold_disk_bytes",
        "total_footprint_bytes",
        "memory_saved_bytes",
        "compression_ratio",
        "footprint_compression_ratio",
        "output_mae",
        "output_rmse",
        "output_max_abs",
        "output_rmse_reduction_vs_pq_only",
        "score_mae",
        "score_rmse",
        "score_rmse_reduction_vs_pq_only",
        "hot_bytes",
        "warm_pq_bytes",
        "warm_rvq_bytes",
        "rvq_key_active",
        "rvq_value_active",
        "rvq_key_active_fraction",
        "rvq_value_active_fraction",
        "window_start",
        "window_end",
        "hot_tokens_materialized",
        "warm_tokens_materialized",
        "cold_tokens_materialized",
        "reconstructed_tokens",
        "reconstructed_token_fraction",
        "warm_chunk_decodes",
        "cold_chunk_decodes",
        "cold_chunks_touched",
        "cold_chunk_cache_hits",
        "cold_chunk_cache_misses",
    ]
    with output_path.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(results)

    for row in results:
        logger.info(
            "len=%3d profile=%-10s window=%3d mode=%-7s cache=%.3fms recon=%3d warm_decodes=%d cold_decodes=%d drift=%.4f",
            row["sequence_length"],
            row["storage_profile"],
            row["window_tokens"],
            row["quantizer_mode"],
            row["cache_latency_ms"],
            row["reconstructed_tokens"],
            row["warm_chunk_decodes"],
            row["cold_chunk_decodes"],
            row["output_rmse"],
        )

    logger.info("Wrote CSV: %s", output_path)
    return results


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Dense vs tiered-cache evaluation benchmark")
    parser.add_argument(
        "--lengths",
        type=int,
        nargs="+",
        default=[32, 64, 128, 192],
        help="Sequence lengths to evaluate",
    )
    parser.add_argument("--repeats", type=int, default=8, help="Timing repeats per scenario")
    parser.add_argument("--query-count", type=int, default=8, help="Number of decode-style queries per sequence")
    parser.add_argument("--hot-capacity", type=int, default=32, help="Hot-tier token capacity")
    parser.add_argument(
        "--warm-capacity",
        type=int,
        default=None,
        help="Warm-tier token capacity for the hot_warm profile; defaults to max(lengths) - hot_capacity",
    )
    parser.add_argument(
        "--cold-warm-capacity",
        type=int,
        default=None,
        help="Warm-tier token capacity for the cold_mixed profile; defaults to min(64, max(lengths)-hot_capacity-1)",
    )
    parser.add_argument(
        "--context-windows",
        type=int,
        nargs="+",
        default=[32, 64, 128, 0],
        help="Attention window sizes to evaluate; use 0 for the full prefix",
    )
    parser.add_argument(
        "--use-router-for-cold",
        action="store_true",
        help="Use the async router for cold_mixed scenarios when a finite context window is provided",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("./benchmark_outputs/dense_vs_hot_warm.csv"),
        help="CSV output path",
    )
    args = parser.parse_args(argv)

    run_evaluation(
        lengths=args.lengths,
        repeats=args.repeats,
        query_count=args.query_count,
        hot_capacity=args.hot_capacity,
        warm_capacity=args.warm_capacity,
        cold_warm_capacity=args.cold_warm_capacity,
        output_path=args.output,
        context_windows=args.context_windows,
        use_router_for_cold=args.use_router_for_cold,
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())